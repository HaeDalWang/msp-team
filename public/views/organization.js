...
export function renderOrganization() {
  ...
  return `<main class="management-page organization-management"><div class="management-heading"><div><span class="eyebrow">ORGANIZATION ADMIN</span><h1>조직 및 권한 관리</h1><p>본인 이메일·입사일은 우측 상단 설정 → 내 정보 수정에서 변경할 수 있습니다. 관리자는 구성원 수정에서 근무시간과 조직 정보도 관리합니다.</p></div><div class="heading-actions">${canEdit ? `<button id="part-manager-toggle">파트 관리</button><button id="member-add" class="primary" ${state.busy ? 'disabled' : ''}>구성원 추가</button>` : ''}</div></div>
