...
export function renderOrganization() {
  ...
  return `<main class="management-page organization-management"><div class="management-heading"><div><span class="eyebrow">ORGANIZATION ADMIN</span><h1>조직 및 권한 관리</h1><p>입사일·근무시간은 관리자 계정에서 각 구성원의 ‘수정’ 버튼으로 설정합니다. 입사일이 빠른 순서로 발표합니다.</p></div><div class="heading-actions">${canEdit ? `<button id="part-manager-toggle">파트 관리</button><button id="member-add" class="primary" ${state.busy ? 'disabled' : ''}>구성원 추가</button>` : ''}</div></div>
