...
export function renderCompLeave() {
  ...
  return `<main class="management-page comp-leave-management"><div class="management-heading"><div><span class="eyebrow">COMPENSATORY LEAVE</span><h1>대체휴가 관리</h1><p>승인된 초과근무를 시간 단위로 적립하고, 휴가 사용 신청을 승인하면 차감합니다.</p></div><span>${isAdminOrLead() ? '관리자·팀장 검토 가능' : '본인 기록 작성'}</span></div>
    ...
    <label>고객사 또는 업무명<input data-overtime="customer" required maxlength="200" value="${h(draft.customer)}"></label><div class="comp-form-row time compact-time"><label>시작 시간<input data-overtime="startTime" type="time" required value="${h(draft.startTime)}"></label><label>종료 시간<input data-overtime="endTime" type="time" required value="${h(draft.endTime)}"></label><div class="calculated-hours"><span>산정 예상</span><strong id="calculated-hours">${calculateHours(draft.startTime, draft.endTime)}시간</strong></div></div><small>종료 시간이 시작보다 이르면 다음 날 종료로 계산합니다. 최종 시간은 서버에서 산정됩니다.</small>
    ...
    <section class="comp-register-panel"><header><h2>내 대체휴가 사용 신청</h2></header><p>승인 전에는 잔여 시간이 차감되지 않습니다. 최종 승인 시 잔여 시간을 확인합니다.</p><form id="leave-form" class="holiday-form"><label>사용 날짜<input data-leave="date" type="date" required value="${h(state.leave.date)}"></label><label>사용 시간<input data-leave="hours" type="number" min="0.01" max="24" step="0.01" required value="${h(state.leave.hours)}"></label><label>사유<input data-leave="reason" required maxlength="2000" value="${h(state.leave.reason)}"></label><button class="primary" ${state.busy ? 'disabled' : ''}>사용 신청</button></form></section>
