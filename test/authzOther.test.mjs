...
test('POST /api/customers allows an engineer to add a customer for a teammate (shared ownership churn)', async () => {
  const database = { query: async (sql) => (sql.includes('INSERT INTO customers') ? { rows: [{ id: 1 }] } : { rows: [] }) }
  const server = createApp(database, env).listen(0)
  await new Promise((resolve) => server.once('listening', resolve))
  try {
...
test('POST /api/customers rejects an unauthenticated request', async () => {
